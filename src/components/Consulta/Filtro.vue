<template>
    <div class="flex justify-start items-center pb-10">
      ...
      <select-base
        ...
        @change="getDocsByType(filtro.tipoDocumento)"
      />
      <select-base
        label=""
        class="text-center w-48 ml-36"
        id="filtrosDocs"
        :options="filtro.listDocsFiltrados"
        v-if="filtro.listDocsFiltrados !== '' && filtro.tipoDocumento !== ''"
      ...
      <button-base
        ...
        v-if="filtro.listDocsFiltrados !== '' && filtro.tipoDocumento !== ''"
        @click="saveFiltro(filtro.filtroDocValue, filtro.tipoDocumento)"
        :disabled="filtro.filtroDocValue === ''"
      />
      <button-base
        label="Aplicar"
        class="ml-40 border-gray text-black hover:bg-white hover:text-red"
        v-if="filtro.tipoDocumento === '4'"
