<template>
  <table class="border border-solid border-t-0 border-l-0  border-black border-collapse text-gray-900 w-full">
    <tr>
      <th v-for="(header, index) in headers" :key="index" class="p-2 border-b-2 border-l-0" :class="{'border-r-2': header.label === 'Convenio Modificatorio'}">
        {{ header.label }}
      </th>
    </tr>
    <tr id="rows" class="" v-for="(item, index) in data" :key="index" :class="{ ' bg-gray-100': index % 2 === 0 }">
      <td v-for="(header, index) in headers" :key="index" class="p-2 text-center" :class="{'border-r-2 text-center': header.field === 'convenio_modificatorio', 'border-l':index == 0 }">
        <p v-if="header.field === 'contrato'" @click="detalleContrato()">
          DGDFM-03-21
          {{ item[header.field] }}
        </p>
        <p v-if="header.field === 'convenio_modificatorio'" @click="detalleContrato()">
          CONVENIO-1
          {{ item[header.field] }}
        </p>
        {{ item[header.field] }}
...
export default {
  ...
  setup() {
    ...
    const detalleContrato = () => {
      console.log('Se muestran los detalles del contrato')
    }
    const openActions = (id) => {
      document.getElementById(id).classList.toggle('hidden')
    }

    return { dots, openActions, detalleContrato }
  },
}
</script>
